...
get_ipython().system('wget https://www.dropbox.com/s/pxcz2wdz04zxocq/CamVid.zip?dl=1 -O ./content/CamVid.zip')
get_ipython().system('unzip -o ./content/CamVid.zip -d ./content')
...
get_ipython().system('wget https://github.com/iArunava/ENet-Real-Time-Semantic-Segmentation/raw/master/datasets/CamVid/ckpt-enet.pth -O ./content/ckpt-enet-CamVid-downloaded.pth')
...
inputImg = plt.imread('./content/test/' + imageFileName)
...
trueLabels = Image.open('./content/testannot/' + imageFileName)
trueLabels = cv2.resize(np.array(trueLabels), (512, 512), cv2.INTER_NEAREST)
...
outArray = outTensor.cpu().detach().numpy()
...
b_ = outTensor.data.max(0)[1].cpu().numpy()
...
true_seg = decode_segmap(trueLabels)
